    ...
    if (races) {
        const racesPromise = races.map(async (raceId) => {
            try {
                ...
                // continue the trace of the request that started the race
                const parentCtx = propagation.extract(context.active(), race.traceCarrier ?? {}) ;
                await context.with(parentCtx, () => tracer.startActiveSpan('race-engine.run' , async (span) => {
                    try {
